...
class AIControlRunner(RunnerBase):
    ...
    def _build_send_tick_outputs(self) -> List[dict]:
        ...
        if has_new:
            # config 추가 지연: 측정 처리시간(now_robot-last_obs)에 이 초를 더해 "더 오래 걸린 것처럼"
            # 동작 → start_index(스트라이드)가 그만큼 앞으로 감. 예) 처리시간 0.068 + 설정 0.1 = 0.168처럼.
            # 기본 0.0(끔). 명령 전달 지연 보정용. (측정 처리시간 경로에만 적용; fixed_pad 오버라이드엔 미적용.)
            extra_latency = max(0.0, float(getattr(cfg, "send_extra_latency_sec", 0.0)))
            fixed_pad = getattr(cfg, "observation_send_padding_sec", None)
            if fixed_pad is not None:
                padding_sec = max(0.0, float(fixed_pad))
            else:
                padding_sec = max(0.0, now_robot - last_obs + 0.1 + extra_latency)
