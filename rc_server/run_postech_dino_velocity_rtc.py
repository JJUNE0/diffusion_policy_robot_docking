...
# ---------------- RTC helpers (from inference.py) ----------------
def build_exponential_blend_schedule(horizon, gamma=0.6):
    idx = np.arange(horizon, dtype=np.float32)
    beta = 1.0 - np.exp(-gamma * idx)
    if horizon > 1:
        beta = beta / max(beta[-1], 1e-8)
    return np.clip(beta, 0.0, 1.0)


def compute_ranking_weights(horizon, gamma=0.35):
    idx = np.arange(horizon, dtype=np.float32)
    w = np.exp(-gamma * idx)
    return w / (w.sum() + 1e-8)
...
def shift_previous_path(prev_path):
    shifted = np.zeros_like(prev_path)
    shifted[:-1] = prev_path[1:]
    shifted[-1] = prev_path[-1]
    return shifted
...
def apply_exponential_continuity_smoothing(selected_action, prev_selected_action=None, blend_gamma=0.6):
    if prev_selected_action is None:
        return selected_action

    horizon = selected_action.shape[0]
    beta = build_exponential_blend_schedule(horizon, gamma=blend_gamma)
    prev_shifted = shift_previous_path(prev_selected_action)

    smoothed = np.zeros_like(selected_action)
    for i in range(horizon):
        b = beta[i]
        smoothed[i] = b * selected_action[i] + (1.0 - b) * prev_shifted[i]
...
def _preprocess_snapshot(window_snapshot, device):
    ...
    room1_history_full, latest_room1_rgb = _extract_recent_image_history(window_snapshot, "video1", obs_horizon, device)
    room2_history_full, latest_room2_rgb = _extract_recent_image_history(window_snapshot, "video2", obs_horizon, device)
...
def postech_inference(window_snapshot, latency_marks, polaris_config: AIControlConfig):
    global _PREV_SELECTED_ACTION

    logger.info("Inference with POSTECH Codes")
    args = _load_model_once()
    dt = 1.0 / max(1, polaris_config.inference_fps)
    n_samples = int(getattr(args, "num_samples", 1))
    ranking_gamma = float(getattr(args, "ranking_gamma", 0.35))
    blend_gamma = float(getattr(args, "blend_gamma", 0.60))
    w_ang = float(getattr(args, "continuity_w_ang", 2.0))
    ...
    # CHANGED: handle n_samples == 1 cleanly
    if n_samples == 1:
        selected_action = samples[0]
        best_idx = 0
    else:
        selected_action_raw, best_idx, _ = select_best_sample_with_ranking(
            samples=samples,
            prev_selected_action=_PREV_SELECTED_ACTION,
            ranking_gamma=ranking_gamma,
            w_ang=w_ang,
        )
        selected_action = apply_exponential_continuity_smoothing(
            selected_action=selected_action_raw,
            prev_selected_action=_PREV_SELECTED_ACTION,
            blend_gamma=blend_gamma,
        )
        _PREV_SELECTED_ACTION = selected_action.copy()
    ...
    for i in range(num_steps):
        ...
        # TODO: Robot 쪽에서 받는 velocity trajectory
        # TODO: AI가 내뱉어낸 velocity trajectory
        # TODO - 1: 두 trajectory 비교해서 정상적으로 value가 들어가는지 확인
        # TODO - 2: 정상적으로 들어가고 있다면, 이 값을 Encoder가 실제로 잘 추종하는지 확인
        # command_steps.append(CommandStep(dx=v * dt, dy=0.0, dtheta=w * dt, dt=accumulated_dt)) # position
        command_steps.append(CommandStep(dx=v * dt, dy=0.0, dtheta=w * dt, dt=dt)) # velocity
