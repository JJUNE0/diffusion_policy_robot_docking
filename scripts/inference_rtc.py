...
def main(args):
    ...
    for step in range(ep_steps):
        ...
        if n_samples == 1:
            selected_action = res[0]
            best_idx = 0
        else:
            selected_action_raw, best_idx, ranking_scores = select_best_sample_with_ranking(
                samples=res,
            ...
            prev_selected_action = selected_action.copy()
        ...
        all_selected_indices.append(best_idx)

        if (step + 1) % 10 == 0 or (step + 1) == ep_steps:
            print(f"진행: {step + 1} / {ep_steps} steps")
    ...
    ax_vel[0].plot(steps, sel_v, color='forestgreen', linewidth=1.2, label='Selected linear vel')
    ...
    ax_vel[1].plot(steps, sel_w, color='forestgreen', linewidth=1.2, label='Selected angular vel')
    ...
    plt.tight_layout()
    save_vel_path = os.path.join(output_dir, "velocity_comparison_per_step_80000_test_in_3.png")
    ...
    ax.plot(ai_path[:, 0], ai_path[:, 1], color="forestgreen", linestyle="-", linewidth=1.0, alpha=0.8,
            label="AI RTC (room1 + room2 + velocity)")
    ...
    ax.scatter(ai_path[-1, 0], ai_path[-1, 1], c="forestgreen", s=80, marker="X", zorder=5, label="AI RTC End Point")

    ax.set_title(
        f"Room1 + Room2 + Velocity + RTC Open-Loop Prediction\n(Vision stride: {vision_stride}, Samples: {n_samples})",
        fontsize=14,
        pad=15,
    )
    ...
    plt.tight_layout()
    save_path = os.path.join(output_dir, "Step_80000_test_in_3.png")
