...
def build_spec() -> NodeSpec:
    sending = os.environ.get("ENABLE_SEND", "true").lower() in ("1", "true", "yes")
    return NodeSpec(
        name="ai-control",
        label="AI Control (도킹)",
        tags=["ai", "control"],
        ...
        control_lease=sending,            # dry-run(ENABLE_SEND=false)이면 제어권 미점유
...
def main() -> None:
    ...
    logger.info("=== 설정된 AI 모델(플러그인): %s ===", getattr(config, "run_plugin", "Unknown"))
